...
{
    public Queen(List<int> coordinates, int team) : base(coordinates, team)
    {
    }
    ...
    public override bool[,] GetLegalMoves(List<int> _coordinates, int team, Tile[,] board)
    {
        ...
        while (r < 8)
        {
            if (board[r, c].IsEmpty() == true) legal[r, c] = true;
            else { if (board[r, c].GetPiece().GetTeam() != team) legal[r, c] = true; break; }
            r++;
        }
